...
void ClientConnection::waitForRequests() {
    ...
    while (!exit) {
        ...
        if (COMMAND("USER")) {
            ...
            char* ip[4];
            uint16_t port;
            int sockid = openListenPort(port, ip);
            if (sockid < 0) {
                ...
                fprintf(fd, "227 Entering Passive Mode (%s,%s,%s,%s,%i,%i).\n", ip[0], ip[1], ip[2], ip[3], port % 256,
                        port / 256);
                fflush(fd);
                if (acceptIncomingConnection(sockid) < 0)
                    fprintf(fd, "501 Syntax error in parameters or arguments.\n");
            }
...
int ClientConnection::openListenPort(uint16_t &port, char **ip) {
    if (data_socket != -1)
        close(data_socket);
    ...
    ip[0] = std::strtok(ip_bin, ".");
    ip[1] = std::strtok(nullptr, ".");
    ip[2] = std::strtok(nullptr, ".");
    ip[3] = std::strtok(nullptr, ".");
...
int ClientConnection::acceptIncomingConnection(int sockid) {
    sockaddr_in address{};
    socklen_t len;
